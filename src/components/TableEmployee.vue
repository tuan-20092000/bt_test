<template>
  <div class="wrap-table">
    <div class="table">
      <table>
        ...
        <thead>
          <tr>
            <th
              style="min-width: 40px; height: 46px"
              class="check"
              id="check-all"
            >
              <input type="checkbox" name="" id="" />
            </th>
            <th style="min-width: 150px; height: 34px" id="">MÃ NHÂN VIÊN</th>
            <th style="min-width: 250px; height: 34px" id="">TÊN NHÂN VIÊN</th>
            <th style="min-width: 120px; height: 34px" id="">GIỚI TÍNH</th>
            <th style="min-width: 150px; height: 34px" id="">NGÀY SINH</th>
            <th style="min-width: 200px; height: 34px" id="">SỐ CMND</th>
            <th style="min-width: 250px; height: 34px" id="">CHỨC DANH</th>
            <th style="min-width: 250px; height: 34px" id="">TÊN ĐƠN VỊ</th>
            <th style="min-width: 150px; height: 34px" id="">SỐ TÀI KHOẢN</th>
            <th style="min-width: 250px; height: 34px" id="">TÊN NGÂN HÀNG</th>
            <th style="min-width: 250px; height: 34px" id="">
        ...
        <tbody>
          <tr v-for="(employee, index) in employeeList" :key="index" v-on:click="active(index)" v-on:dblclick="edit(index)">
            ...
            <td v-bind:class="{'selected' : (index == selectedRow)}">{{ employee.employeeCode }}</td>
            <td v-bind:class="{'selected' : (index == selectedRow)}">{{ employee.fullName }}</td>
            <td v-bind:class="{'selected' : (index == selectedRow)}">{{ convertGender(employee.gender) }}</td>
            <td v-bind:class="{'selected' : (index == selectedRow)}">{{ convertDate(employee.dateOfBirth) }}</td>
            <td v-bind:class="{'selected' : (index == selectedRow)}">{{ employee.identityCard }}</td>
            <td v-bind:class="{'selected' : (index == selectedRow)}">{{ employee.position }}</td>
            <td v-bind:class="{'selected' : (index == selectedRow)}">{{ employee.companyName }}</td>
            <td v-bind:class="{'selected' : (index == selectedRow)}">{{ employee.accountNumber }}</td>
            <td v-bind:class="{'selected' : (index == selectedRow)}">{{ employee.bankName }}</td>
            <td v-bind:class="{'selected' : (index == selectedRow)}">{{ employee.branch }}</td>
            <td v-bind:class="{'selected' : (index == selectedRow)}" class="sticky border-left">
              <div class="col-edit">
    ...
    <div class="footer">
      <div class="count-record">Tổng số: {{countEmployee.length}} bản ghi</div>
      <div class="right-footer">
        <select v-model="countEmployeePerPage" @change="changeCountPerPage" name="" id="">
        ...
        <div 
          v-for="page in countPage" :key="page" 
...
export default {
  ...
  methods: {
    ...
    changeCountPerPage(){
      this.employeeList = this.countEmployee.slice(0, this.countEmployeePerPage);
      this.countPage = Math.floor(this.countEmployee.length / this.countEmployeePerPage) + 1;
      this.selectedPage = 1; 
    },
